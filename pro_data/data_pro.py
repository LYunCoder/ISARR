...
def load_data_and_labels(train_data, valid_data, user_review, item_review, user_rid, item_rid, stopwords):
    ...
    reid_user_train = [] # [[item_id1,item_id2,...][item_id11,...]...] every element is same as the list of u_rid
    reid_item_train = [] #[[user_id1, user_id2,...][user_id11,user_id22,...]...]
    uid_train = []
    iid_train = []
    y_train = []
    u_text = {} # {userid:[[w1ofr1,w2ofr1,...][w1ofr2,w2ofr2,...]...]}save each user's words of all reviews he/she commented.
    u_rid = {} #{userid:[int(item1),int(item2),...]}
    i_text = {}
    i_rid = {}
    i = 0
    for line in f_train:
    ...
    u_len = x[int(0.9 * len(review_num_u)) - 1] # only covering 90% numbers of reviews
    review_len_u = np.array([len(j) for i in u_text.values() for j in i])# the number of words of each review
    x2 = np.sort(review_len_u)
    u2_len = x2[int(0.9 * len(review_len_u)) - 1] # only covering 90% numbers of words
